 ...
 * 检查一段文本是否含有违法违规内容
 */
WeChat.prototype.msgSecCheck = function(msg){
    var that = this;
    return new Promise(function(resolve,reject){
        that.getAccessToken().then(function(data){
            //格式化请求连接
            var url = util.format(that.apiURL.msgSecCheck,that.apiDomain,data);
            
            //使用 Post 请求创建微信菜单
            that.requestPost(url,JSON.stringify({
                content: msg
            })).then(function(data){
                  //将结果打印
                //   console.log(data);
 ...
 * 校验一张图片是否含有违法违规内容
 */
WeChat.prototype.imgSecCheck = function(media) {
    var that = this;
    return new Promise(function(resolve, reject){
        that.getAccessToken().then(function(data){
            var url = util.format(that.apiURL.imgSecCheck,that.apiDomain,data);
            request.post({
                url,
                formData: {
                    media: media
